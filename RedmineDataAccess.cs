...
namespace combit.RedmineReports
{
    abstract class RedmineDataAccessBase : IDisposable
    {
        ...
        public DataProviderCollection GetRedmineData(string projectId, string sqlCommand, int startDate)
        {
            ...
            DataTable issueHistory = CreateIssueHistory(projectId, sqlCommand, startDate);
            DataTable changeSetTable = CreateChangeSetTable(projectId, startDate);
            DataTable openTicketTimeSpan = GetOpenTicketTimeSpan(projectId, sqlCommand);
            return collection;
        }
         * */

        public void GetRedmineData(string projectId, string sqlCommand, DateTime fromDate, DateTime toDate)
        {
            IDbCommand cmd = _connection.CreateCommand();
            ...
            DataTable issueHistory = CreateIssueHistory(projectId, sqlCommand, fromDate, toDate);
            DataTable changeSetTable = CreateChangeSetTable(projectId, fromDate, toDate);
            DataTable openTicketTimeSpan = GetOpenTicketTimeSpan(projectId, sqlCommand);
